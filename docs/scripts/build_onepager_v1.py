...
def slide_onepager(prs):
    ...
    add_text(slide, 0.33, 0.56, 8.0, 0.32,
             "AI-Guided Legacy Web Application Migration  —  at a glance",
    ...
    # ─── THE SOLUTION + FLOW ──────────────────────────────────────
    section_label(slide, 2.86, "THE SOLUTION", TEAL)
    add_text(slide, CONTENT_L + 0.02, 3.24, CONTENT_W, 0.32,
             "A Claude Code plugin that turns a legacy rewrite into a repeatable, "
             "auditable, team-scale workflow — all state lives in git.",
    ...
    benefits = [
        ("Consistency + a gate",
         "One workflow; /verify enforces lint, types, tests & behavioural parity before done."),
        ("Durable git ledger",
         "Full audit trail of who-did-what, plus one-command /rollback of any unit."),

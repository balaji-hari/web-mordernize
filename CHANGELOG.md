...
### Added
- **Cross-repo migrations, via a gitignored per-developer local file.** The legacy (source) codebase and the new (target) codebase can now live in different folders/repos — previously the plugin hard-assumed both lived in the same working directory. The actual legacy path is a **machine fact, not a team decision**, so it is never git-tracked: `migration.md §1` gains a yes/no "Legacy source in a separate repo/folder?" toggle (team-wide, git-shared), and `/init` writes a tracked template `.claude/modernize/source_root.local.json.example`; a developer whose legacy repo lives elsewhere copies it to the gitignored `.claude/modernize/source_root.local.json` and sets their own path there (see `skills/_shared/source-root-resolve.md` for the resolution rule, referenced by every consuming skill instead of restated inline). `/web-modernize:analyze` resolves and validates the local file when present, and records only the team-wide `state.uses_external_source` boolean (no path) plus `state.source_repo` provenance (remote + commit — safe to share, unlike the path). Every consumer that reads legacy files — `legacy-analyzer`, `unit-migrator-subagent`, `parity-reviewer`, `migration-critic`, `cross-cutting-migrator`, and `/web-modernize:scaffold`'s legacy-asset copy — resolves `source_paths[]` and asset/stylesheet scans against the resolved root instead of assuming the working directory. `hooks/guard-legacy.mjs` gains matching dual-root logic (its own fail-safe semantics, separate from its `state.json` read: a missing/unparseable local file means "no external source," never "allow every write"): it protects the external source root from stray writes and, in that mode, treats the whole target repo as new-app workspace (skipping the same-repo "deny non-zoned subdirectories" rule, which would otherwise false-block legitimate writes). No local file (the default) leaves every existing same-repo behaviour byte-for-byte unchanged.

### Schema
- **Additive only — `schema_version` unchanged at 4.** New optional top-level `state.json` properties `uses_external_source` (boolean) and `source_repo` ({ remote, root_commit }). No path is ever stored in git-tracked state. `unit.schema.json` is untouched (`source_paths[]` keeps its existing meaning, now documented as "relative to the resolved source root" in the consuming agents). No migration required for existing `.claude/modernize/` state.
...
### Added
- **Datastore-reachability preflight for `/foundation`** — a new Preflight step 5 checks the target datastore is reachable *before* the consolidated design gate, gated on the concern set including `data` or `auth` using a local password store (skipped for external-IdP auth with no local DB). It reuses the migration tool's own read-only status command (`alembic current` / `dotnet ef migrations list` / `./mvnw flyway:info` / `npx prisma migrate status` or `npx typeorm migration:show`) rather than a hardcoded per-engine probe ladder, falls back to a plain TCP connect on `host:port` parsed from the (masked) connection string, and degrades to a graceful `⊘ could not verify` as a last resort — never a fabricated `✅`. Surfaces one readiness line in the design gate and, on `⚠ unreachable`, asks via `AskUserQuestion` (default recommendation: stop, mirroring `/scaffold`'s missing-runtime posture) whether to proceed anyway or fix infra first. The per-stack `Apply:` / `Status:` commands move out of `skills/foundation/SKILL.md` into a new `## Data migration` section on each of the 6 target-api framework files (`fastapi`, `spring-boot-3`, `dotnet-minimal-api`, `nestjs`, `express`, `hono`); the "Run DB migrations" step now reads `Apply:` from there instead of a hardcoded per-stack list.
- **`## Verify commands` section on all 6 target-api framework files** — lint/typecheck/test commands per stack (`ruff`/`mypy`/`pytest` for FastAPI, Maven checkstyle-or-spotless/compile/test for Spring Boot, `dotnet format`/`dotnet build`/`dotnet test` for .NET, the existing `npm` shape for NestJS/Express/Hono, applied uniformly), matching the "framework file is the source of truth" convention already used for `## Scaffold`/`## Test framework`/`## Dev server`. `skills/scaffold/SKILL.md`'s "Update verify.config.json" step now populates the `api` block from it (graceful-degrade to the current Node defaults when a framework has no such section or the target is unknown) — closes the gap where a non-Node API target inherited nonsense Node defaults.
- **`EALLOWSCRIPTS` entry in `agents/permanent-gotchas.md`** — a new, symptom-triggered "Sandbox / environment" section: *if* `npm install` fails with `EALLOWSCRIPTS`, unset `npm_config_allow_scripts` / `NPM_CONFIG_ALLOW_SCRIPTS` for that command. Root cause: the Claude Code sandbox's global `.npmrc` `allow-scripts=@anthropic-ai/claude-code` line gets rejected by npm's `resolve-allow-scripts.js` for any project-scoped install. Applies to any Node-based scaffold run inside such a sandbox, and only where that `.npmrc` line is actually set.
- **`/status` "Foundation" rollup** — a new read-only section 4 (following sections renumbered 5-11) fires only when `state.foundation.concerns` is set and `state.status == "scaffolded"`; reads each `units/__<concern>__.json` and prints per-concern status plus the failure diagnostic when blocked, so a resumed session can tell "foundation never started" from "foundation half-done, blocked on infra" without opening every unit file by hand.
- **Standing SQL-injection warning in `legacy-analyzer`** — a new "Warnings to surface" entry for string-concatenated / dynamically-built SQL (VB.NET/C# concatenation, PHP interpolation, Java `Statement` vs `PreparedStatement`) so it's reliably raised as a `state.open_decisions[]` entry at `/plan` time instead of depending on an agent noticing it per-unit opportunistically. Framed as earlier visibility / defense-in-depth, not a new safety net — `parity-reviewer` already catches injection as a `security_*` finding and blocks `/verify` on unacknowledged highs regardless of whether this warning fires. `skills/plan/SKILL.md`'s cross-cutting-decision "Typical examples" list now names string-built SQL explicitly so it reliably lands as an `open_decision`.

### Fixed
- **Redundant `dotnet new sln` in `frameworks/dotnet-minimal-api.md`** — `## Test framework` now reuses an existing repo-root `.sln`/`.slnx` if one exists (the common case when migrating an existing .NET solution in place) and only creates a new one if genuinely none exists, instead of unconditionally running `dotnet new sln` and colliding with a pre-existing solution file.
- **i18n false-positive guard** — `skills/foundation/SKILL.md`'s i18n discovery row and `agents/cross-cutting-migrator.md`'s i18n concern now confirm more than one locale/culture is actually defined (multiple `App_LocalResources` culture suffixes, multiple `.properties` locales, multiple `$translate`/`<fmt:message>` catalogs) before treating i18n as genuinely in scope — a single-locale resource-file convention is a labeling convention, not real localization, and shouldn't trigger a full i18n provider/catalog design.

### Version note
All seven items are prompt/doc-only: new framework-file sections, a new `permanent-gotchas.md` entry, a new read-only `/status` section, and warning/discovery-guard wording — no schema field changes, no command renames, no `schema_version` bump.
...
### Added
- **`unit-migrator` subagent conversion** — `agents/unit-migrator.md` is now a real Claude Code subagent (`model: sonnet`), launched via the `Agent` tool by `/next`/`/migrate`/`/retry` instead of being loaded inline. The file splits into **Part A** (caller preflight: collision handling, acquisition, the plan gate, open-decisions resolution — run inline, interactive) and **Part B** (the subagent body — design and/or execute, never both interactively). Since a subagent cannot pause mid-task for human approval, a **gated** unit now runs Part B **twice**: a `call_mode: "plan_only"` call that designs and returns a plan without writing anything, then — after `[a]pprove`/`[r]evise`/`[c]ancel` — a fresh `call_mode: "full"` call that executes the approved plan. Each call is independent and fire-and-forget (same shape as `parity-reviewer`); the approved-plan call re-reads the unit's own source files once (bounded, single-unit cost) rather than relying on unverified subagent-resume semantics. The subagent writes target/test/E2E files and `notes/<id>.md` directly but never `units/<id>.json` — the calling skill writes that file's terminal record from the subagent's structured return value. Bounds each unit's context to roughly constant size instead of letting N units' history accumulate across a session.
- **`/web-modernize:next-batch [--n=K]`** (`skills/next-batch/SKILL.md`, new) — migrates up to `K` (default 3, max 8) independent pending units in parallel via `workflows/next-batch.js`, which fans out one `unit-migrator` (`call_mode: "full"`) subagent per unit. **Always skips the per-unit plan-approval gate** (parallel human review of K plans doesn't compose) — states this upfront; use `/next` for reviewed, one-at-a-time migration. Brings the user-facing command count to **19**.
- **CSS audit** — three additive pieces shipped together: (1) a "Styling detection" pass in `agents/legacy-analyzer.md` (also wired into `workflows/analyze-discovery.js`'s detect phase) reports CSS frameworks/preprocessors, stylesheet-vs-CSS-in-JS approach, a rule-count estimate, and shared stylesheets referenced by more than one entry point, into a new additive `analysis.json.styling` object; (2) `/web-modernize:plan` gains **Step 2c**, which offers to size detected shared stylesheets as an explicit `kind: "shared"` unit (`__shared-styles__`) — reusing the existing shared-unit mechanism rather than a new kind or phase gate; (3) `agents/migration-critic.md` gains a "CSS fidelity" review lens with four new `quality_findings[].kind` values (`css_unported_rules`, `css_specificity_leakage`, `css_missing_breakpoint`, `css_dead_selector`), added to `templates/unit.schema.json`'s enum — advisory, surfaced via the existing `/verify` + `/quality-check`, no new gating.
- **`/verify` Workflow-tool parallelization** — `workflows/verify-run.js` (new) runs per-unit verification as a `pipeline()` (no barrier between units — one unit's reviewers can run while another is still running its test suite) with a `parallel()` fan-out, per unit, of the three independent reviewer dimensions (`parity-reviewer`, `migration-critic`, the opt-in dynamic tier) once that unit's lint/typecheck/test thresholds pass. `skills/verify/SKILL.md` gains the same Method A (Workflow, preferred) / Method B (sequential, today's original behaviour as the fallback) framing already used by `/analyze` and `/foundation`. The workflow only returns data (its reviewer agents stay read-only); the skill writes every `units/<id>.json` and runs the unchanged project-wide post-checks.

### Version note
Every schema change is additive (`analysis.json.styling`, four `css_*` quality-finding kinds) — `schema_version` stays `3`. `next-batch` is a new opt-in command; no existing command's default behaviour changes. `unit-migrator`'s subagent conversion changes internal execution shape, not its observable per-unit outcome (`migrated`/`failed`/`pending`-cancelled) or any schema field.

## [0.16.0] - 2026-07-01

Closes a cluster of gaps surfaced by a real dry run (ASP.NET WebForms → React/Vite + .NET Minimal API): E2E specs were never auto-generated, `/report` showed no verification depth, CSS/visual fidelity needed a manual diff, missing config keys broke silently, and a few legacy paradigms leaked through. All schema changes are additive (`schema_version` stays `3`); the `/report` default change is behavioural, not breaking. Every fix is a shape-agnostic rule or generic mechanism — nothing is hard-coded to the demo app.

### Added
- **Auto-authored per-unit E2E specs** — `agents/unit-migrator.md` gains **§7d**: when dynamic testing is enabled (`verify.config.json.dynamic.enabled`) and the unit has a UI surface, it authors one Playwright spec per unit at `apps/web-new/e2e/<unit.id>.spec.ts` (keyed by unit id, not route), driven by the unit's `routes[]` + its Given/When/Then behaviour contract. **Author-only — never runs** (running stays in `/verify --dynamic`), never blocks. The specs **assert visually load-bearing facts** (image `src` resolution via `naturalWidth > 0`, key legacy elements/classes visible) — catching the broken-asset/dropped-chrome class that build + static parity review miss. Additive `unit.e2e` object in `templates/unit.schema.json` (`spec_path`, `authored_count`, `routes_covered[]`, `authored_at`, nested `e2e_results`) — **no `schema_version` bump**.
- **E2E run results captured** — `/web-modernize:verify --dynamic` (step 5c Phase B) now parses the Playwright summary and records `unit.e2e.e2e_results = {passed, failed, skipped, ran_at}`. Still advisory, still never blocks.
- **`/report` surfaces verification depth** — new **Pending verification** (migrated-but-not-verified units + *why*: unacknowledged parity highs, coverage below threshold, never parity-reviewed, missing/placeholder E2E, recorded failures), **Dynamic / E2E results** (per-unit authored count + pass/fail/skip + dynamic findings), and **Behavioural parity & code-quality findings** sections — in all three formats. New `{{PENDING_VERIFICATION_TABLE}}`, `{{DYNAMIC_RESULTS_TABLE}}`, `{{PARITY_FINDINGS_BLOCK}}`, `{{QUALITY_FINDINGS_BLOCK}}` placeholders + JSON keys (`pending_verification[]`, `dynamic_results[]`, `parity_summary{}`, `quality_summary{}`).
- **CSS/visual fidelity & silent-config guards** — two new `agents/permanent-gotchas.md` entries: *a config-referenced value missing from the target config breaks silently* (the broken-image / wrong-base-URL class — neither build nor static parity catches it) and *a migration that compiles can still look wrong* (port/compare the legacy CSS, don't re-style). `unit-migrator` §7b gains a "verify config-driven values resolve" rule (the static counterpart to §7d's asset assertion).
- **`state.open_decisions[]`** (additive) — `/web-modernize:plan` now separates cross-cutting **architectural decisions** (e.g. one responsive layout vs. a separate mobile tree, state-management, routing strategy) from generic open questions and records them for an explicit team decision at the approval gate; `unit-migrator` **refuses to resolve an open one unilaterally** mid-migration and writes the resolution back. Closes the "mobile strategy got decided silently during a unit" class.
- **`duplication` quality dimension** — `migration-critic` flags duplicated exported symbols (copy-pasted DTOs/types, validation fns) across a unit's files or repeated from an earlier unit, pointing at the shared location to extract/import from. Added to the `quality_findings[].kind` enum (additive); `unit-migrator`'s reuse rule now explicitly covers shared types/DTOs.

### Changed
- **`/web-modernize:report` defaults to HTML** when no `--format` is given (was Markdown). `--format=md|json` remain available.
- **Foundation gates DB migrations before seeding** — `/web-modernize:foundation` now runs the target's DB-migration step **before** dev-user seeding; a failed migration is a **loud blocker** (marks `__auth__` failed, does not advance `state.status`). A still-missing users table after migrations no longer silently finalizes — it surfaces a prominent `⚠ AUTH SCHEMA NOT READY` callout instead of a buried skip reason.
- **Reactive auth state** — `cross-cutting-migrator`'s auth concern now establishes a **reactive** auth context/store (re-renders on mid-session login/logout), and `unit-migrator` consumes it reactively rather than via synchronous render-body reads.
- **SPA-navigation idioms** — `unit-migrator` translates server redirects (`Response.Redirect`, `RedirectToAction`, `sendRedirect`, carried-over `window.location.href`) to the target's client router instead of full-page reloads; `migration-critic` flags surviving full-page navigation as legacy-paradigm leakage.

### Version note
All schema changes are additive (`unit.e2e`, `state.open_decisions[]`, the `duplication` quality kind) — `schema_version` stays `3`, existing state/unit files validate unchanged. The `/report` default-format change is behaviour, not a breaking schema change. No new skills/agents/frameworks (only edits to existing).

## [0.15.0] - 2026-06-29

Several feature areas, plus one structural rename: (1) a **plan-approval gate** (*present the plan → wait for explicit approval → then write*, applied with judgment — it gates code generation, not read-only/bookkeeping work); (2) **background / non-UI unit support** (jobs, schedulers, queue consumers, hubs, batch processors); (3) **emergent shared-code extraction + cross-unit rollback safety**; and (4) the **Foundation phase** — `/web-modernize:foundation` generalizes and **replaces `/auth`**, establishing all confirmed cross-cutting concerns (auth always, plus opt-in i18n / feature flags / error handling / telemetry / logging) as the first slice. The schema changes are all additive (`schema_version` stays `3`), but **renaming the `/auth` command and the `auth_done` phase → `/foundation` / `foundation_done` is a breaking change** — see the version note below. Pull with `/plugin uninstall web-modernize && /plugin install web-modernize`.

### Added
- **Opt-out per-unit plan gate** — `/web-modernize:next`, `:migrate`, and `:retry` now present the unit's plan (target file layout, approach & key decisions, tests to write, dependencies, open questions) and **wait for `[a]pprove` / `[r]evise` / `[c]ancel` before writing any target file**. The gate lives once in `agents/unit-migrator.md` (new **§3.5**, between deciding the target layout and writing files); cancelling returns the unit to `pending` with nothing written (not a failure). The migration-wide default is **opt-out (gate ON)**.
- **`state.review_mode`** (additive optional field in `templates/state.schema.json`, declared under `properties` with `additionalProperties` still `false` — **no `schema_version` bump**) — `plan-first` (default, also assumed when absent) | `auto`. Set **optionally at `/web-modernize:plan`** via `--review-mode=plan-first|auto` (aliases `--auto` / `--plan-first`) or an optional `Review mode:` line in `migration.md §6`; **sticky** across re-plans. Whatever is set is the default for the **complete** migration. `/plan`'s closing banner reports the active mode.
- **Per-unit override flags** — `--plan` (force the gate for one unit even when `review_mode: auto`) and `--no-plan` (skip it for one unit even when `review_mode: plan-first`) on `/next`, `/migrate`, `/retry`. The skills pass a `plan_override` to `unit-migrator`, which resolves it against `state.review_mode` in one place.
- **Always-on consolidated design gate on `/web-modernize:foundation`** — presents the design for all cross-cutting concerns (auth + any opted-in) and waits for approval **before writing any code**, independent of `review_mode`, skippable only with `--no-plan`. See the Foundation-phase section below.

### Added — Background / non-UI units
- **`background` unit kind + `trigger` field** — `templates/unit.schema.json` gains `background` in the `kind` enum and an optional `trigger` (`scheduled` | `queue` | `hub` | `batch` | `startup`). Additive (a new enum value + optional field) — existing unit files validate unchanged, **no `schema_version` bump**. Models legacy code that runs *without an HTTP request*: scheduled jobs, queue/message consumers, realtime hubs, batch/file processors, startup daemons.
- **Non-route discovery pass in `legacy-analyzer`** — a separate cross-stack sweep (`BackgroundService`/`IHostedService`, Hangfire/Quartz, `@Scheduled`, MSMQ/RabbitMQ/Kafka/Service Bus consumers, SignalR `Hub`s, file-watchers/ETL, `Program.Main` daemons) emits `kind: "background"` entries with a `trigger`. **Exempt from the 100-entry importance cap** so a nightly billing job is never silently outranked by 100 pages.
- **Background migration recipe in `unit-migrator`** — translates the *trigger* to the target stack's idiomatic mechanism (cron/serverless, BullMQ/`@Cron`/APScheduler/`BackgroundService`, the target's queue consumer, realtime layer, worker CLI, or startup hook), preserving schedule/queue/ack semantics. Prefers a `## Background jobs` section in the target `frameworks/<api>.md` when present.
- **Build + tests-only smoke gate for background units** — no endpoint to curl, no page to build, and the job is **never invoked** at verify time (real side effects / infra). The gate runs a compile/build check + the unit's scoped tests + coverage, records `smoke.kind = "background-tests-only"`, and prints an **explicit, non-silent** note that functional smoke was skipped.

### Added — Emergent shared-code extraction & cross-unit rollback safety
- **`extracted_shared[]` on units (additive) + `/plan` backfill** — when `unit-migrator` extracts reusable code mid-migration that wasn't seeded as a unit, it places it in the target stack's conventional shared location (inferred, **confirmed at the plan gate** — no hard-coded `src/lib/`), records it in notes, and appends `{id, path, purpose}` to its own `extracted_shared[]` (concurrency-safe). `/plan`'s new **Step 6b** backfills a `kind: shared` unit (`status: migrated` — the code exists) for each, wires the extracting unit's `depends_on`, dedups, and warns on independent duplicates. Closes the "emergent shared code silently duplicated" gap.
- **Cross-unit rollback safety (`/rollback --force-shared`)** — `/rollback` now runs a **data-driven** shared-file check (unit `kind` `shared`/`cross-cutting`/synthetic, `extracted_shared[].path`, paths shared across units, the `notes/__layout__.md` record — never path-pattern matching) and **refuses by default** when a unit owns shared files that live dependents rely on, printing the blast radius. `--force-shared` overrides and records `rollback_info.shared_impact`. Closes the "rolling back a unit reverts the layout/shared utils out from under others" gap.

### Added — Foundation phase (replaces `/auth`)
- **`/web-modernize:foundation`** — generalizes the synthetic-`__auth__` pattern into a command that establishes ALL the team's cross-cutting concerns as the foundational slice: **auth always**, plus any of i18n / feature flags / error handling / telemetry / logging opted into in the new **`migration.md §13`** checklist. `/plan` reads §13, **confirms the set with the developer**, records `state.foundation.concerns[]`, and seeds one synthetic unit per concern (`__auth__` `kind: service`; others `kind: cross-cutting`; phase 1, pending). `/foundation` discovers each concern, shows **one consolidated always-on design gate**, then implements all concerns — **Method A** fans out the new `agents/cross-cutting-migrator.md` one-per-concern in parallel via the new `workflows/foundation-establish.js` (each writes disjoint files, returns composition-root wiring), **Method B** is the sequential fallback; the skill then **wires the shared composition root once, sequentially**, smoke-builds, seeds dev users (auth), finalizes each concern unit `migrated`, and flips `scaffolded → foundation_done`. Auth keeps its bespoke logic (dev-user seeding, hashing gotchas) as the auth concern.
- **Schema (additive):** `unit.kind` gains `cross-cutting`; `state.status` gains `foundation_done` (`auth_done` kept, deprecated); a new optional `state.foundation` object holds the concern set. No `schema_version` bump.
- **Removed `skills/auth/SKILL.md`** (superseded by `skills/foundation/SKILL.md`). `auth_done → foundation_done` ripples through `/next`, `/migrate`, `/retry`, `/scaffold`, `unit-migrator`, and `templates/plan.md`.

### Added — Integration command, data foundation, static perf review & dynamic testing tier
- **`/web-modernize:integrate`** — new command that **incrementally** assembles the migrated units into a composed app: reconciles a central router + nav from each unit's `routes[]` (inferring from target files for older units), runs a **whole-app smoke**, flags **orphaned units**, reports cutover coverage %, and — for `strategy: strangler-fig` — generates/refreshes the traffic-splitting proxy (migrated routes → new app, rest → legacy). **Idempotent** — runnable at any stage and as the final cutover (`--final`); `--dry-run` reports without writing. Additive `unit.routes[]` (populated by `unit-migrator`) + `state.integration` object; new `## Integration` recipe section in `frameworks/*.md`. Its shared router/nav/proxy files inherit the cross-unit rollback protection.
- **`data` foundation concern (wiring only)** — `/foundation` can now establish the **data-access wiring** (ORM/client + connection/pool config + migration-tooling harness) as a concern (opt-in via `migration.md §13`). It explicitly does **NOT** translate tables/queries/stored procs — that bulk work remains a separate later phase (`docs/planning/future-data-layer-migration.md`). Adds `data` to `state.foundation.concerns` + a `cross-cutting-migrator` recipe.
- **Static performance-regression review** — `migration-critic` gains a static-perf dimension flagging N+1 / per-row lazy-load (`perf_n_plus_one`), unbounded/unpaginated queries (`perf_unbounded_data`), request waterfalls (`perf_waterfall`), blocking I/O on hot paths (`perf_blocking`), and bundle bloat (`perf_bundle`) — added to the `quality_findings[].kind` enum. Advisory, never blocks; surfaces through the existing `/verify` step 5b + `/quality-check`. No new agent.
- **Dynamic testing tier (opt-in, `/web-modernize:verify --dynamic`)** — a new advisory step 5c runs **Phase A API replay** (replay recorded legacy fixtures vs the new API, diff → `dynamic_api_replay` findings) and **Phase B Playwright E2E** (`dynamic_e2e` findings). **Never blocks** (E2E flakiness must not gate). Companion `--capture-baseline` records the legacy baseline. Opt-in via `migration.md §12` "Dynamic testing" → `/scaffold` installs Playwright + a replay harness + writes `verify.config.json.dynamic`. Additive `unit.dynamic_findings[]` + `dynamic_reviewed_at`; new `## Dynamic tests` framework recipe. Phase C visual diff is out of scope.

### Changed
- **`migration.md`** gains an optional `Review mode:` line in §6 and a new optional **§13 Cross-cutting concerns** checklist (neither is a REQUIRED field — `/plan`'s validation list is unchanged).
- **`/web-modernize:plan`** now also confirms the cross-cutting concern set with the developer (§13), seeds the foundation synthetic units, records `state.foundation.concerns[]`, and runs **Step 6b** to backfill `kind: shared` units from `extracted_shared[]`.
- **`/web-modernize:plan`** resolves and persists `review_mode` (flag → `migration.md §6` → existing value → `plan-first`) and reports it in the closing banner. It also carries `trigger` onto background units and **does not auto-add `__auth__`** to a background unit's `depends_on` (jobs run without a user session); background units default to a late phase (admin/batch) unless the dependency graph pulls them earlier.

### Version note — additive schema, but one BREAKING rename
Everything schema-side is additive (`review_mode`, `background`/`trigger`, `cross-cutting`, `foundation_done`, the `foundation` object, `extracted_shared[]`, `rollback_info.shared_impact` — `schema_version` stays `3`, existing state/unit files validate unchanged). **However, replacing the `/auth` command with `/foundation` and the `auth_done` phase with `foundation_done` is a breaking change** (a removed/renamed command + a renamed phase value). By strict semver this warrants a **major** bump; it is acceptable now only because the plugin has **no production users**. When versioning is next addressed this release should be cut as the major bump, and teams mid-migration on the old `/auth` flow would need a fresh `/init`.

## [0.13.0] - 2026-06-28

Adds exhaustive discovery, knowledge capture, drift detection, and a dependency view — and introduces the `workflows/` orchestration pattern. All additive: no `state.schema.json` / `unit.schema.json` change, no removed/renamed commands. Pull with `/plugin uninstall web-modernize && /plugin install web-modernize`; **no `.claude/modernize/` reset required**.

### Added
- **`workflows/analyze-discovery.js`** — the plugin's first Workflow-tool script. A `Detect` pass (one `legacy-analyzer` for framework + metadata) plus a `Discover` phase that fans out `legacy-analyzer` **loop-until-dry** (parallel scoped rounds, dedup by id, budget-guarded) until two consecutive rounds find nothing new. Returns the same `analysis.json` shape with a far more complete `entry_points[]` than a single pass produces on a large estate (the analyzer caps at ~100 per pass). Agents are read-only; the skill writes the file.
- **Optional Given/When/Then behaviour contract in notes** — `templates/notes-template.md` gains an optional `## Behaviour contract (Given/When/Then)` section. `unit-migrator` populates it from the legacy unit's rules (calculations, validations, eligibility, defaults, state transitions) when they exist — left empty for trivial units — and `parity-reviewer` reads it as the spec when source and target disagree. Makes the extracted semantics inspectable and git-tracked instead of living only in one run's context.
- **Dependency graph in `plan.md`** — `/web-modernize:plan` renders a Mermaid graph of unit `depends_on` (subgraphed by phase) into `plan.md` via a new `{{DEPENDENCY_GRAPH}}` placeholder. Structural / status-agnostic (everything is `pending` at plan time); it surfaces parallelizable vs bottleneck units for standup assignment. Collapses to phase-level with a printed note when a plan exceeds 40 units (no silent truncation).

### Changed
- **`skills/analyze/SKILL.md`** — "Detection strategy" is now **Method A / Method B**: Method A invokes `workflows/analyze-discovery.js` via the Workflow tool (authorized by the `/analyze` invocation) when available; Method B is the existing single `legacy-analyzer` pass as a graceful fallback. The `analysis.json` schema is unchanged — Method A just returns a more complete `entry_points[]`, so `/plan` is unaffected.
- **`skills/status/SKILL.md`** — new **Staleness** section using **git commit time** (`git log -1 --format=%ct`, not mtime — git doesn't preserve mtimes across clone/pull): flags `analysis.json` or `migration.md` committed after `plan.md` and nudges a `/plan` re-run. Skips silently when files are untracked. Read-only; no state change.
- **`agents/unit-migrator.md`, `agents/parity-reviewer.md`** — the migrator populates the behaviour contract; the reviewer consumes it as spec.
...
Adds the disciplines and one agent that harden *understanding, verification rigor, and safety* around the migration, without touching the per-unit execution loop that is this plugin's strength. All changes are additive: no `state.schema.json` `schema_version` bump, no removed/renamed commands. Teams pull this with `/plugin uninstall web-modernize && /plugin install web-modernize`; **no `.claude/modernize/` reset required**.

### Added
- **`agents/migration-critic.md`** — new read-only subagent that reviews a migrated unit's TARGET code for **idiomatic quality**, orthogonal to `parity-reviewer`'s behavioural check. Flags "JOBOL" / legacy-paradigm leakage (WebForms-in-React `useEffect`-postback emulation, jQuery-style imperative DOM in a reactive framework, scriptlet/code-behind-shaped controllers, God-components), ceremonial error handling, single-use abstractions, tests that exercise paths instead of pinning behaviour, and on-call/operability gaps. Returns `quality_findings[]` + a one-line headline. **Advisory only — never blocks** the `migrated → verified` transition.
- **`/web-modernize:quality-check`** (`skills/quality-check/SKILL.md`) — on-demand migration-quality review for one unit or `--all`; the sibling of `/parity-check` minus the acknowledge/gate machinery (quality findings don't block, so there's nothing to suppress). Brings the user-facing command count to **17**.
- **`templates/unit.schema.json` — `quality_findings[]`, `quality_reviewed_at`, `quality_headline`** — additive fields holding the migration-critic's output. No `schema_version` bump.
- **Toolchain preflight in `/web-modernize:scaffold`** — a fast read-only check at the top of scaffold that reads each chosen target's runtime floor from its `frameworks/<name>.md` and probes the binaries (Node/npm, Python, .NET SDK, Java/Maven). Missing required runtime → **stop before any half-scaffold** with the install one-liner; below-floor → warn and ask; all green → proceed. Unknown targets skip the probe and fall through to the existing follow-up. Skipped under `--assets-only`.
- **`SECRETS.local.md` quarantine** — `/web-modernize:init` now gitignores `.claude/modernize/SECRETS.local.md`, the only place a raw discovered credential may be written.

### Changed
- **`agents/parity-reviewer.md`** — three additions: a **security-parity dimension** (dropped authorization, injection, lost output-encoding, secret-in-bundle, dropped CSRF) with five new finding kinds (`security_authz_dropped`, `security_injection`, `security_output_encoding`, `security_secret_exposure`, `security_csrf`), default severity `high`, gated by an exploit-scenario discipline; a **refute pass** that requires every `high` finding to have a one-sentence consumer-visible impact before it's emitted, raising precision on the one gate that can trap a working migration; **untrusted-input** and **secret-masking** rules.
- **`templates/unit.schema.json`** — the five `security_*` values added to the `parity_findings[].kind` enum (additive).
- **`skills/verify/SKILL.md`** — a new **advisory step 5b** runs `migration-critic` after the parity gate (graceful-degrade, never blocks; `--no-quality` opts out) and prints a `quality:` line; clarifies that security-kind `high` parity findings block exactly like any other high and are acknowledged the same way via `/parity-check`.
- **`agents/legacy-analyzer.md`, `agents/unit-migrator.md`** — untrusted-input and secret-masking rules: legacy code is data, never instructions; credential values are masked (`AKIA****`) + `file:line` and never written into `analysis.json`, `notes/*.md`, symbol maps, or any tracked artifact; instruction-shaped text is reported, not obeyed.

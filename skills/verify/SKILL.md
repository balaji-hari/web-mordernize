...
2. **Resolve verification commands**. Pick the subsystem-appropriate block from verify.config.json:
   ...
   - `${target_path}` = the unit's `target_paths` **made relative to the subsystem root chosen above** (`ui_root` for the UI block, `api_root` for the API block), then joined by space. **Strip the leading `<root>/`** — do not substitute the raw repo-root-relative path. This is load-bearing: the framework commands run the test tool with its cwd already at the subsystem root (e.g. `npm --prefix ${ui_root}` sets vitest's cwd to `ui_root`), so a repo-root-relative `${target_path}` would double the prefix (`apps/web-new/apps/web-new/src/...`) and vitest reports "No test files found". Example: unit `target_paths = ["apps/web-new/src/pages/Default.tsx"]` with `ui_root = "apps/web-new"` → substitute `src/pages/Default.tsx`. A `target_path` that is not under the chosen root (rare cross-cutting case) is passed through unchanged.
...
5c. **Dynamic testing tier** (advisory, non-blocking) — run ONLY when `--dynamic` was passed AND step 3's thresholds were met. This is the higher-fidelity, runtime counterpart to the static parity/quality reviews; it **never blocks** the transition (E2E flakiness must not gate a migration). Read `verify.config.json.dynamic`:
   ...
   - **Phase A — API replay** (when `dynamic.api_replay` is set and the unit touches the API): if `dynamic.baseline_dir` has recorded legacy fixtures, run the `api_replay` command (substitute `${target_path}` — using the **same subsystem-root-relative rule as step 2** — plus `${api_root}` and `${baseline_dir}`) — a harness that replays the recorded legacy requests against the new API and diffs responses. Each diff becomes a finding `{ kind: "dynamic_api_replay", severity, observation, recommendation }`. If `baseline_dir` is empty/missing, **skip Phase A** with: `No baseline fixtures — run /web-modernize:verify --capture-baseline first. Skipping API replay.`
   - **Phase B — E2E** (when `dynamic.e2e` is set and the unit touches the UI): run the `e2e` command (Playwright) scoped to this unit's routes/flows — prefer the unit's authored spec (`unit.e2e.spec_path`, written by `unit-migrator` §7d) when present. When substituting `${target_path}` or the spec path into a `--prefix`-based command, apply the **same subsystem-root-relative rule as step 2** (the Playwright cwd is `ui_root` via `--prefix`, so a repo-root-relative `apps/web-new/e2e/<id>.spec.ts` would double to `apps/web-new/apps/web-new/e2e/...`; pass `e2e/<id>.spec.ts`). A failing step becomes a finding `{ kind: "dynamic_e2e", severity, observation }`. **Also parse the Playwright run summary and record the pass/fail/skip counts** into `unit.e2e.e2e_results = { passed, failed, skipped, ran_at: <now> }` (create the `e2e` object if the unit predates §7d authoring). This is what `/web-modernize:report` surfaces as real run results.
